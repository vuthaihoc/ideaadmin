						...
						<tbody>														
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>
										<form id="lang_active_cal" action="{{ url(config('laraadmin.adminRoute') . '/module_field_lang_active') }}">
									...
									<td style="max-width:300px;"><?php echo BackendHelper::parseValues($field['popup_vals']) ?></td>
...
$(function () {
	...
	$('.Switch.Ajax.Lang').click(function() {
		...
		$.ajax({
			type: "POST",
			url : "{{ url(config('laraadmin.adminRoute') . '/module_field_lang_active') }}",
			data : {
				_token: '{{ csrf_token() }}',
