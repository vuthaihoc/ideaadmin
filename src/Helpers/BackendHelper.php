...
namespace Idea\Ideaadmin\Helpers;
...
/**
 * Class BackendHelper
 * @package Idea\Ideaadmin\Helpers
 *
 * This is IdeaAdmin Helper class contains methods required for Admin Panel functionality.
 */
class BackendHelper
{
    /**
     ...
     *
     * $names = BackendHelper::generateModuleNames($module_name);
     *
    ...
    /**
     * Get list of Database tables excluding IdeaAdmin Context tables like
     ...
     *
     * $tables = BackendHelper::getDBTables([]);
     *
     ...
     *
     * $modules = BackendHelper::getModuleNames([]);
     *
     ...
     *
     * BackendHelper::parseValues($field['popup_vals']);
     *
     ...
     *
     * BackendHelper::log("info", "", $commandObject);
     *
    ...
    public static function recurse_copy($src, $dst)
    {
        ...
        while(false !== ($file = readdir($dir))) {
            if(($file != '.') && ($file != '..')) {
                if(is_dir($src . '/' . $file)) {
                    BackendHelper::recurse_copy($src . '/' . $file, $dst . '/' . $file);
                } else {
                    // ignore files
            ...
            foreach($objects as $object) {
                if($object != "." && $object != "..") {
                    if(is_dir($dir . "/" . $object))
                        BackendHelper::recurse_delete($dir . "/" . $object);
                    else
                        unlink($dir . "/" . $object);
                }
            }
     ...
     *
     * $password = BackendHelper::gen_password();
     *
     ...
     *
     * BackendHelper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
     *
     ...
     *
     * BackendHelper::print_menu_editor($menu)
     *
...
//        }
        ...
        $childrens = \Idea\Ideaadmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();
        
        if(count($childrens) > 0) {
            $str .= '<ol class="dd-list">';
            foreach($childrens as $children) {
                $str .= BackendHelper::print_menu_editor($children);
            }
            $str .= '</ol>';
        }
        $str .= '</li>';
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li' . $treeview . ' ' . $active_str . '><a href="' . url(config("laraadmin.adminRoute") . '/' . $menu->url) . '"><i class="fa ' . $menu->icon . '"></i> <span>' . BackendHelper::real_module_name($menu->name) . '</span> ' . $subviewSign . '</a>';
        ...
        if(count($childrens)) {
            ...
            foreach($childrens as $children) {
                ...
                if(Module::hasAccess($module->id)) {
                    $str .= BackendHelper::print_menu($children);
                }
            }
            $str .= '</ul>';
        }
     ...
     * This needs to be done recursively
     *
     * BackendHelper::print_menu_topnav($menu)
     *
     * @param $menu menu array from database
    ...
    public static function print_menu_topnav($menu, $active = false)
    {
        ...
        $str = '<li ' . $treeview . '' . $active_str . '><a ' . $treeview2 . ' href="' . url(config("laraadmin.adminRoute") . '/' . $menu->url) . '">' . BackendHelper::real_module_name($menu->name) . $subviewSign . '</a>';
        
        if(count($childrens)) {
            $str .= '<ul class="dropdown-menu" role="menu">';
            foreach($childrens as $children) {
                $str .= BackendHelper::print_menu_topnav($children);
            }
            $str .= '</ul>';
        }
        $str .= '</li>';
     ...
     *
     * BackendHelper::getLineWithString()
     *
     ...
     *
     * BackendHelper::getLineWithString2()
     *
    ...
    public static function setenv($param, $value)
    {
        ...
        $envfile = BackendHelper::openFile('.env');
        $line = BackendHelper::getLineWithString('.env', $param . '=');
     ...
     *
     * BackendHelper::get_migration_file("students_table");
     *
