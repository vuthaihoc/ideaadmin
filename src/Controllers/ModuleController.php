...
namespace Idea\Ideaadmin\Controllers;
 ...
 * @package Idea\Ideaadmin\Controllers
 *
 */
class ModuleController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        // Delete Controller
        \File::delete(app_path('/Http/Controllers/Backend/' . $module->name . 'Controller.php'));
        ...
        while(BackendHelper::getLineWithString($file_admin_routes, "Backend\\" . $module->name . "Controller") != -1) {
            $line = BackendHelper::getLineWithString($file_admin_routes, "Backend\\" . $module->name . 'Controller');
        ...
        if(BackendHelper::getLineWithString($file_admin_routes, "=== " . $module->name . " ===") != -1) {
            $line = BackendHelper::getLineWithString($file_admin_routes, "=== " . $module->name . " ===");
            $fileData = file_get_contents($file_admin_routes);
            $fileData = str_replace($line, "", $fileData);
            file_put_contents($file_admin_routes, $fileData);
        }
    ...
    public function generate_update($module_id)
    {
        ...
        $module_lang = Module::where('name', $module->name.'_langs')->first();

        if(!empty($module_lang)){
            $field = ModuleFields::where('module', $module_id)->get();
            if(!empty($field)) {

                foreach ($field as $field_item) {
                    $field_item_lang = ModuleFields::where('colname', $field_item->colname)->where('module', $module_lang->id)->first();

                    if($field_item->lang_active == 1){
                        //Nếu chưa có thì thêm mới
                        if(!isset($field_item_lang->id)){
                            $field_item->module_id = $module_lang->id;
                            $field_id = ModuleFields::createField($field_item);
                            // Give Default Full Access to Super Admin
                            $role = \App\Role::where("name", "SUPER_ADMIN")->first();
                            Module::setDefaultFieldRoleAccess($field_id, $role->id, "full");
                        }
                        //Nếu có thì cấp lại quyền
                        else {
                            $field_item_lang->lang_active = 1;
                            $field_item_lang->save();

                            // Give Default Full Access to Super Admin
                            $role = \App\Role::where("name", "SUPER_ADMIN")->first();
                            Module::setDefaultFieldRoleAccess($field_item_lang->id, $role->id, "full");
                        }
                    }
                    //Nếu không tạo thì vô hiệu hoá quyền và cập nhật lại trạng thái hiển thị
                    else if(isset($field_item_lang->id)) {
                        $field_item_lang->lang_active = 0;
                        $field_item_lang->save();
                        // Give Default Full Access to Super Admin
                        $role = \App\Role::where("name", "SUPER_ADMIN")->first();
                        Module::setDefaultFieldRoleAccess($field_item_lang->id, $role->id, "readonly");
                    }
                }
            }
    ...
    /**
     * Get Array of all Module Files generated by IdeaAdmin
     *
     * @param Request $request
     * @param $module_id Module ID
     * @return \Illuminate\Http\JsonResponse
    ...
    public function get_module_files(Request $request, $module_id)
    {
        ...
        $arr = array();
        $arr[] = "app/Http/Controllers/Backend/" . $module->controller . ".php";
