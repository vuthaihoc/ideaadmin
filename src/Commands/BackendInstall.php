 ...
 *
 * Command to install IdeaAdmin package into project which moves lot of file from 'src/Installs' directory to Project
 */
class BackendInstall extends Command
{
    ...
    protected $description = 'Install IdeaAdmin Package. Generate whole structure for /admin.';
    ...
    /**
     * Generates and Moves files to install IdeaAdmin package.
    ...
    {
        try {
            $this->info('IdeaAdmin installation started...');
            
            $from = base_path('vendor/idea/ideaadmin/src/Installs');
            $to = base_path();
            ...
            if($this->confirm("Want to set your Database config in the .env file ?", true)) {
                ...
                if(BackendHelper::laravel_ver() == 5.5) {
                    ...
                    BackendHelper::setenv("DB_HOST", $db_data['host']);
                    BackendHelper::setenv("DB_PORT", $db_data['port']);
                }
                ...
                BackendHelper::setenv("DB_DATABASE", $db_data['db']);
                BackendHelper::setenv("DB_USERNAME", $db_data['dbuser']);
                BackendHelper::setenv("DB_PASSWORD", $db_data['dbpass']);
            }
            
            if(env('CACHE_DRIVER') != "array") {
                config(['cache.default' => 'array']);
                BackendHelper::setenv("CACHE_DRIVER", "array");
            }
            ...
            if($this->confirm("This process may change/append to the following of your existing project files:"
                ...
                $this->replaceFolder($from . "/app/Controllers/Backend", $to . "/app/Http/Controllers/Backend");
                if(BackendHelper::laravel_ver() == 5.5) {
                ...
                if ($this->confirm('Would you like to customize this url ?', false)) {
                    ...
                    $arline = BackendHelper::getLineWithString($to."/config/laraadmin.php", "'adminRoute' => 'admin',");
                ...
                */
                ...
                // la-assets
                $this->line('Generating IdeaAdmin Public Assets...');
                ...
                // $this->call('db:seed', ['--class' => 'IdeaAdminSeeder']);
                ...
                if(BackendHelper::getLineWithString('config/database.php', "dump_command_path") == -1) {
                    ...
                    $mysqldriverline = BackendHelper::getLineWithString('config/database.php', "'driver' => 'mysql'");
                ...
                if(BackendHelper::laravel_ver() == 5.5) {
                    if(BackendHelper::getLineWithString($to . "/routes/web.php", "require __DIR__.'/admin_routes.php';") == -1) {
                        $this->appendFile($from . "/app/routes.php", $to . "/routes/web.php");
                    }
                    $this->copyFile($from . "/app/admin_routes.php", $to . "/routes/admin_routes.php");
                } else {
                    if(BackendHelper::getLineWithString($to . "/app/Http/routes.php", "require __DIR__.'/admin_routes.php';") == -1) {
                ...
                if(BackendHelper::getLineWithString($to . "/gulpfile.js", "mix.less('admin-lte/AdminLTE.less', 'public/la-assets/css');") == -1) {
                ...
                $this->info("\nIdeaAdmin successfully installed.");
            ...
            if(strpos($msg, 'SQLSTATE') !== false) {
                throw new Exception("BackendInstall: Database is not connected. Connect database (.env) and run 'la:install' again.\n" . $msg, 1);
            } else {
                $this->error("BackendInstall::handle exception: " . $e);
                throw new Exception("BackendInstall::handle Unable to install : " . $msg, 1);
            }
        }
    }
    ...
    private function copyFolder($from, $to)
    {
        ...
        BackendHelper::recurse_copy($from, $to);
    }
    ...
    private function replaceFolder($from, $to)
    {
        ...
        if(file_exists($to)) {
            BackendHelper::recurse_delete($to);
        }
        BackendHelper::recurse_copy($from, $to);
    }
